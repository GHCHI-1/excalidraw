...
const SubscriptionModal = ({ userId, token, onAccessGranted }) => {
    ...
    const checkAccess = async () => {
        if (!userId || !token) {
            setMessage('Please register or log in to access collaboration.');
            setIsOpen(true);
            return;
        }
        try {
            const response = await axios.post(
                `${process.env.NEXT_PUBLIC_API_URL}/check-access`,
                { user_id: userId },
            ...
            if (response.data.access) {
                ...
                setMessage('Trial used. Subscribe to continue (10 CNY/month).');
                setIsOpen(true);
                const paymentResponse = await axios.post(
                    `${process.env.NEXT_PUBLIC_API_URL}/create-payment`,
                    { user_id: userId, amount: 10 },
                    { headers: { Authorization: `Bearer ${token}` } }
                );
                if (isMobile && paymentResponse.data.redirect_url) {
            ...
            setMessage('Error checking access. Try again.');
            setIsOpen(true);
        }
    };
        ...
        const interval = setInterval(async () => {
            try {
                const response = await axios.get(
                    `${process.env.NEXT_PUBLIC_API_URL}/subscription-status/${orderId}`,
                    { headers: { Authorization: `Bearer ${token}` } }
                );
